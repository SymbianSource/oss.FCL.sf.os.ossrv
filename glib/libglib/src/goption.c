...
PLS(q,g_option_error_quark,GQuark)
#define q (*FUNCTION_NAME(q,g_option_error_quark)())
 ...
 * Since: 2.6
 */
EXPORT_C void g_option_context_free (GOptionContext *context) 
{
 ...
 * Since: 2.6
 */
EXPORT_C void g_option_context_set_help_enabled (GOptionContext *context,
                                        gboolean        help_enabled)
	  ...
	  if ((*argv)[i][0] == '-' && (*argv)[i][1] != '\0' && !stop_parsing)
		...
		{ /* short option */
		  ...
		  arg = (*argv)[i] + 1;
			...
			nulled_out = g_new0 (gboolean, arg_length);
...
{
  return FIX_CASTING(char *)dgettext (domainname, msgid);
}
