...
PLS(quark_floating ,gparam,GQuark)
#define quark_floating  (*FUNCTION_NAME(quark_floating ,gparam)())
...
g_param_spec_ref (GParamSpec *pspec)
{
  ...
  g_atomic_int_inc (FIX_CASTING(int *)&pspec->ref_count);
...
g_param_spec_unref (GParamSpec *pspec)
{
  ...
  is_zero = g_atomic_int_dec_and_test (FIX_CASTING(int *)&pspec->ref_count);
		       ...
		       GParamFlags  flags)
{
  ...
  if (flags & G_PARAM_STATIC_NAME)
    {
      pspec->name = FIX_CASTING(char *)g_intern_static_string (name);
